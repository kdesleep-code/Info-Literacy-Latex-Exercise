...
LaTeXでは、箇条書きを作るために次の3種類の環境(\begin{}~\end{}で囲むやつ)がよく使われます。

| 環境名 | 表示形式 | 用途の例 |
|---------|-----------|----------|
| `itemize` | 「・」のような黒丸付きリスト | 一般的な箇条書き |
| `enumerate` | 「1.」「2.」などの番号付きリスト | 手順や順序のある説明 |
| `description` | 用語＋説明の組み合わせ | 用語解説や略語の定義など |
